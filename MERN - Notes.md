...
14. Add Login Route to Auth Route (`routes/api/auth.js`):

    - Bring in `bcrypt`, `jwt`, `config`, and `express-validator`:
    ...
    - Add Login Route Logic: This is very similar to the register endpoint, but you only need `email` and `password` and not `name`:
